-- Fix Function Search Path Mutable warning by updating all functions to have secure search_path
-- Update all existing functions to set search_path to empty string for security
...
CREATE OR REPLACE FUNCTION public.check_rate_limit_with_lockout(p_identifier text, p_attempt_type text, p_max_attempts integer DEFAULT 5, p_window_minutes integer DEFAULT 15, p_block_minutes integer DEFAULT 30, p_lockout_attempts integer DEFAULT 10, p_lockout_duration_minutes integer DEFAULT 60)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
