  ...
  return (
    <div className="space-y-1 text-sm text-gray-600 mb-3">
      {customerInfo.filter(info => info.label !== 'Name').map((info, index) => {
        // Hide sensitive data (phone, address) but show location and match indicators
        if (info.label === 'Phone' || info.label === 'Address') {
          return (
            <div key={index} className="flex items-start gap-1 text-xs text-green-600">
              <span>✓ {info.label} information available</span>
            </div>
          );
        }
